...
export const businessAPI = rootAPI.injectEndpoints({
    endpoints: builder => ({
        ...
        deleteImage: builder.mutation({
            query: ({businessId, id}) => ({
                url: `${BASE_URL}${BUSINESS_API}${businessId}/image/${id}/`,
                method: 'DELETE'
            })
        }),
        // Image upload endpoints (signed-URL flow)
        initializeImageUpload: builder.mutation({
            query: ({businessId, type, contentType, filename, generateThumbnail}) => ({
                url: `${BASE_URL}${BUSINESS_API}${businessId}/upload/${type}/`,
                method: 'POST',
                params: { generateThumbnail: generateThumbnail ? 'true' : 'false' },
                body: { contentType, filename }
            })
        }),
        confirmImageUpload: builder.mutation({
            query: ({businessId, imageId}) => ({
                url: `${BASE_URL}${BUSINESS_API}${businessId}/upload/confirm/`,
                method: 'POST',
                body: { imageId }
            })
        }),
        // NOTE: Legacy thumbnail endpoints removed (use imageUrl/thumbnailUrl from list APIs)
        // getImageThumbnail, getBusinessThumbnails, getThumbnailsByType - DEPRECATED
        ...
        getMenuItems: builder.query({
            ...
            transformResponse: (items) => {
                ...
                items?.map((item) => {
                    // Extract image URLs from nested image object (backend now includes full image data)
        ...
        getEmployees: builder.query({
            ...
            transformResponse: (items) => {
                const transformedData = [];
                items?.map((item) => {
                    // Use imageUrl from imageDTO instead of base64
                    ...
                    if (image) {
                        image = {
                            ...
                            name: image?.name,
                            status: 'done',
                            url: image?.imageUrl || null, // Use direct URL from backend
                            thumbUrl: image?.thumbnailUrl || null
                        }
                    }
                    transformedData.push({
                        'id': item.id,
                        ...
                        'image': image,
                        'imageId': item.imageId,
                        'imageUrl': image?.url || item.imageUrl || null, // Direct URL for rendering
                        'thumbnailUrl': image?.thumbUrl || item.thumbnailUrl || null
                    })
                })
                return transformedData;
            },
        ...
        getBusinessLogo: builder.query({
            ...
            transformResponse: (logo) => {
                // Use imageUrl from backend instead of base64
                return {
                    uid: logo?.id,
                    name: logo?.name,
                    status: 'done',
                    url: logo?.imageUrl || null, // Direct URL from backend
                    thumbUrl: logo?.thumbnailUrl || null
                }
            }
        }),
...
export const {
    ...
    useInitializeImageUploadMutation,
    useConfirmImageUploadMutation,
    // NOTE: useGetImageThumbnailQuery, useGetBusinessThumbnailsQuery, useGetThumbnailsByTypeQuery - REMOVED
    // Use imageUrl/thumbnailUrl from getBusinessImages instead
