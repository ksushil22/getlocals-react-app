...
/**
 * @deprecated Legacy endpoint removed. Use imageUrl directly from API response.
...
export const getThumbnailUrl = (businessId, imageId) => {
    console.warn('getThumbnailUrl is deprecated. Use imageUrl/thumbnailUrl from API response instead.');
...
export const getAllThumbnailsUrl = (businessId) => {
    console.warn('getAllThumbnailsUrl is deprecated. Use getBusinessImages API instead.');
    const BASE_URL = process.env.BASE_API_URL || '';
...
export const getImageUrl = (businessId, imageId) => {
    console.warn('getImageUrl is deprecated. Use imageUrl from API response instead.');
    const BASE_URL = process.env.BASE_API_URL || '';
    return `${BASE_URL}${PUBLIC_BUSINESS_API}${businessId}/image/${imageId}/`;
};

/**
 * @deprecated Use getDisplaySrc instead.
...
export const getOptimizedImageUrl = (businessId, imageId, useThumbnail = false) => {
    console.warn('getOptimizedImageUrl is deprecated. Use getDisplaySrc instead.');
    return useThumbnail ? getThumbnailUrl(businessId, imageId) : getImageUrl(businessId, imageId);
};

/**
 * Get the display source URL from an image object
 * @param {Object} image - Image object with imageUrl and optional thumbnailUrl
 * @param {string} image.imageUrl - The full image URL
 * @param {string} [image.thumbnailUrl] - The thumbnail URL (optional)
 * @param {boolean} [preferThumbnail=false] - Whether to prefer thumbnail if available
 * @returns {string|null} The URL to display, or null if no valid URL
 */
export const getDisplaySrc = (image, preferThumbnail = false) => {
...
export const getThumbnailDecisionInfo = () => {
    if (typeof window === 'undefined') {
        return { decision: false, reasons: ['Server-side rendering'] };
    }
...
/**
 * Build an image map from an array of images for quick lookup by ID
 * @param {Array} images - Array of image objects from getBusinessImages
 * @returns {Map<string, {imageUrl: string, thumbnailUrl: string|null}>}
 */
export const buildImageMap = (images) => {
    const map = new Map();
...
/**
 * Get image URLs from a map by ID
 * @param {Map} imageMap - Map built by buildImageMap
 * @param {string} imageId - The image ID to look up
 * @returns {{imageUrl: string, thumbnailUrl: string|null}|null}
 */
export const getImageFromMap = (imageMap, imageId) => {
    if (!imageMap || !imageId) return null;
